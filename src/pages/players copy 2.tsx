...
const Players = (props: Props): JSX.Element => {
  ...
  const handleSubmit = (values: any) => {
    ...
    setSummonerIcon(`https://ddragon.leagueoflegends.com/cdn/13.8.1/img/profileicon/${playerInfo.profileIconId}.png`);
    setMatchesIDs(`https://europe.api.riotgames.com/lol/match/v5/matches/by-puuid/${playerInfo.puuid}/ids?start=0&count=20&api_key=${RIOT_API_KEY}`)
    // setMatches(`https://europe.api.riotgames.com/lol/match/v5/matches/by-puuid/${playerInfo.puuid}/?start=0&count=20&api_key=${RIOT_API_KEY}`)

    // https://europe.api.riotgames.com/lol/match/v5/matches/EUW1_6378735601?api_key=RGAPI-12181caf-ffd5-4822-9de4-46ab81afda60
    // console.log(matchesIDs)

    let myMatches: AxiosResponse<any, any>[] = [];
    matchesIDs.map(matchID =>{
      let myMatch = `https://europe.api.riotgames.com/lol/match/v5/matches/${matchID}?api_key=${RIOT_API_KEY}`
      axios.get(myMatch).then((res) =>{
        myMatches.push(res)
      })
